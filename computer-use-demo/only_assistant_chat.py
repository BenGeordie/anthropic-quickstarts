...
def main():
    ...
    output_file = os.path.join(input_dir, "chat-only-assistant.md")
    ...
    try:
        ...
        text = blocks_to_text(groups)

        # Write to output file
        with open(output_file, "w", encoding="utf-8") as f:
            f.write(text)

        print(f"Only-assistant chat saved to: {output_file}")

        print(f"Found {len(groups)} ASSISTANT blocks")
